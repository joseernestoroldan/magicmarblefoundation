...
const MoreArticlesPage = async () => {
  ...
  return (
    <div className="w-full bg-gradient-to-b from-white via-green-300 to-white flex flex-col items-center">
        <h1 className='text-green-950 text-2xl font-medium'>Portal Paddy Field</h1>
        <div className='w-full max-w-5xl mx-auto flex flex-row justify-between gap-4 flex-wrap'>
            {paddyFields.map((item: any, index: number) => {
                return(
                    <Link href={`/paddyfield/${item._id}`} key={index}>
