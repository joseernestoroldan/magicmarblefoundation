...
const SponsorCarousel = ({ sponsors }: SponsorProps) => {
  ...
  return (
    <div className="w-full hidden lg:flex flex-col justify-center items-center space-y-8">
      <h1 className="text-5xl font-semibold text-center text-cyan-500">Sponsor</h1>
        ...
        {sponsors.map((item: any, index: number) => {
          return (
            ...
            >
              ...
              <Link href={`/sponsors/${item._id}`}> <button className="text-cyan-500 text-xl underline font-semibold my-1">Read more about {item.title}</button> </Link>
            </div>
          );
        })}
      </div>
