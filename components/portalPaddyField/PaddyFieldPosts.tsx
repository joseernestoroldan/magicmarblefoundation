  ...
  return (
    <div className="w-full flex flex-col space-y-8">
      {paddyFieldPosts.map((item: any, index: number) => {
        ...
        return (
          <div
            className="w-[90%] h-auto py-4 border-b border-gray-400 flex flex-row"
            key={index}
          >
            ...
            <div className="w-1/2 flex flex-col justify-start items-center space-y-4">
              <h2 className="text-gray-500 text-2xl text-center w-full">
                {item.title}
              </h2>
              <p className=" text-gray-500 text-lg text-justify indent-6 w-[80%]">
                {item.description}
              </p>
              <Link href={`paddyfield/${item._id}`}>
                <button className="text-lg font-semibold underline text-green-500">
                  Read More
                </button>
              </Link>
